  ...
  return (
    <div
      className={`rounded-2xl transition-all duration-200 overflow-hidden ${
        expanded
          ? 'bg-white border-2 border-blue-500 shadow-xl ring-4 ring-blue-500/10'
          : 'bg-white hover:bg-slate-50/40 border border-slate-200 shadow-sm hover:shadow-md'
      }`}
    >
      <div className="p-5 md:p-6">
        {/* 顶部元数据行：分类标签、信源、时间、右侧单一交互按钮 */}
        ...
        {/* 报道大标题：开阔横向展开，大字号、自然排版，支持点击 */}
        <div
          onClick={() => setExpanded(!expanded)}
          className="cursor-pointer group mb-4"
        >
          <h3 className="text-lg md:text-xl font-bold text-slate-900 leading-snug tracking-tight group-hover:text-blue-600 transition-colors">
            {cleanTitle}
          </h3>
        </div>

        {/* 核心结论与传导：合并为优雅通透的双列或卡片，消除多层色块堆挤感 */}
        <div className="space-y-2.5">
          <div className="p-3.5 rounded-xl bg-slate-50 border-l-4 border-blue-600 text-sm md:text-base text-slate-800 leading-relaxed">
          ...
          <div className="p-3.5 rounded-xl bg-emerald-50/50 border-l-4 border-emerald-600 text-sm md:text-base text-slate-800 leading-relaxed">
          ...
          {item.chinaPolicyAngle && (
            <div className="p-3.5 rounded-xl bg-amber-50/50 border-l-4 border-amber-500 text-sm md:text-base text-slate-800 leading-relaxed">
          ...
          <div className="mt-5 pt-5 border-t border-slate-200 space-y-4 animate-in fade-in duration-150">
            ...
            <div className="flex items-center justify-between pt-2">
              ...
              <a
                ...
                className="inline-flex items-center gap-1.5 px-4 py-2 rounded-xl bg-blue-50 hover:bg-blue-100 text-blue-700 border border-blue-200 text-xs md:text-sm font-semibold transition-colors"
              >
                <span>阅读报道原文</span>
