...
export function getMacroInflationTakeaway(title: string, content: string = ''): string {
  ...
  // 专属美债基准与利率曲线突发重定价（带具体点位或高位突破）
  if (/(?:国债|美债|债券).*收益率.*(?:刷新|突破|涨|跌|高位|创|至|报)|(?:10年期|两年期|2年期|5年期|30年期).*收益率/.test(t) || /(?:5\.\d+%|4\.\d+%|基点|bps)/.test(t)) {
    const rateMatch = (title + ' ' + content).match(/(\d+\.\d+[%％])/);
    const rateStr = rateMatch ? `突破${rateMatch[1]}` : '处于阶段高位';
    return `【基准美债重定价与贴现率冲击】：长短端美债收益率快速拉升并${rateStr}，直接推高跨资产无风险贴现率中枢，对权益市场估值中枢与跨国借贷流动性形成约束。`;
  }
...
export function getMacroInflationNextWatchlist(title: string, content: string = ''): string {
  ...
  if (/(?:国债|美债|债券).*收益率|(?:10年期|两年期|2年期|5年期|30年期).*收益率|贴现率/.test(t)) {
